	...
	foreach($data as $datas){
		...
		$total =0;
		   ...
		   if($_POST['to']=="réservation") {
			...
			<div>TVA(%):<br/><input type="number" id="tva" name="tva" value="'.$donns['tva'].'"> <span class="tva">'.$donns['mont_tva'].'</span></div>
	...
	$item_array_id = array_column($_SESSION['add_home'], 'id');	
	  ...
	  if(!empty($_SESSION['add_home'])){
		  ...
		  $total =0;
			...
			<div class="tot">Montant Ajoutée <span class="mont">'.$total.'</span>xof</div>
			</div>';
			echo'</div>';
