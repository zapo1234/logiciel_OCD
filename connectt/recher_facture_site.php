...
$smart_from =($page -1)*$record_peage;
	...
	if($donns['permission']=="user:boss" OR $donns['permission']=="user:gestionnaire"){
		  // emttre la requete sur le fonction
        $req=$bds->prepare('SELECT  date,adresse,check_in,check_out,time,time1,clients,user,montant,montant_repas,mont_tva,types,id_fact,nombre,type,society,code,calls FROM facture WHERE  email_ocd= :email_ocd AND code= :code ORDER BY id_fact DESC LIMIT '.$smart_from.','.$record_peage.'');
